...
namespace CPSC131::BookStore
{
	 ...
	 */
	void BookStore::adjustAccountBalance(int adjustment) { account_balance_ += adjustment; }
	...
	void BookStore::purchaseInventory(const Book &book)
	{
		...
		{
			int oldQuantity = (*it).getStockAvailable();
			(*it).adjustStockAvailable(book.getStockAvailable());
			int newQuantity = (*it).getStockAvailable();
			int adjustment = book.getPriceCents() * (newQuantity - oldQuantity);
	...
	void BookStore::purchaseInventory(
		...
		size_t price_cents, size_t unit_count)
	{
	...
	void BookStore::printInventory()
	{
		...
		for (auto it = bookList.begin(); it != bookList.end(); ++it)
		{
			std::cout << "\"" << (*it).getTitle() << "\", by " << (*it).getAuthor() << " ["
					  << (*it).getIsbn() << "] (" << (*it).getStockAvailable() << " in stock)" << std::endl;
		}
		// std::cout << std::endl; // remove the extra blank line at the end
	}
	 ...
	 */
	// Method to sell a book to a customer using ISBN
	void BookStore::sellToCustomer(Book &book, size_t price_cents, size_t quantity)
	{
		...
		book.adjustStockAvailable(-(int)quantity);	  // Decrease stock available
		adjustAccountBalance(price_cents * quantity); // Increase account balance
	}
}